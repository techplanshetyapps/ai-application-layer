...
async function getOrCreateCollectionId() {
  ...
  // First, look up existing collections to see if our target collection already exists (avoids 405/conflict errors on POST /collections)
  const listRes = await fetch(`${CHROMA_API_URL}/api/v1/collections`, {
    method: 'GET',
...
async function collectionCount() {
  const collectionId = await getOrCreateCollectionId();
  const res = await fetch(`${CHROMA_API_URL}/api/v1/collections/${collectionId}`, {
    method: 'GET',
    headers: await getHeaders(),
  });
  ...
  if (!res.ok) return 0;
  const data = await res.json();
  // Chroma REST GET /collections/{id} returns collection metadata containing count if applicable, 
  // or fallback to querying count via its specific endpoint or returning 0
  if (typeof data.count === 'number') return data.count;

  const countRes = await fetch(`${CHROMA_API_URL}/api/v1/collections/${collectionId}/count`, {
    method: 'GET',
    headers: await getHeaders(),
  });
